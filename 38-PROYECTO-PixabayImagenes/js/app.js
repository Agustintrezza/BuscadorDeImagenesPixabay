        ...
        resultado.innerHTML += `
            <div class="w-1/2 md:w-1/3 lg:w-1/4 mb-4 p-3">
                <div class="bg-white rounded-lg">
                    <img class="w-full rounded-lg" src=${previewURL} alt={tags} />
                    <div class="p-4 bg-white bg-yellow-100 rounded">
                        <p class="card-text text-black"><span class="font-bold">${likes}</span> Me Gusta</p>
                        <p class="card-text text-black"><span class="font-bold">${views}</span> Vistas </p>
        
                        <a href=${largeImageURL} 
                        rel="noopener noreferrer" 
                        target="_blank" class="bg-yellow-500 cursor-pointer w-full p-1 block mt-5 rounded text-center font-ligth uppercase hover:bg-yellow-500 text-black">Ver Imagen</a>
                    </div>
                </div>
            </div>
            `;
...
function mostrarPaginacion() {
    ...
    while( true ) {
        ...
        botonSiguiente.classList.add('siguiente', 'bg-yellow-400', 'px-4', 'py-1', 'mr-1', 'mb-6', 'font-bold', 'uppercase', 'rounded', 'mt-3', 'flex', 'justify-center');
